...
export default function GamePage() {
  ...
  const updateCapturedPieces = useCallback(() => {
    ...
    Object.keys(initialPieces).forEach((pieceType) => {
      const missing =
        (initialPieces as any)[pieceType] * 2 - (currentPieces[pieceType] || 0);
    ...
    moves.forEach((move) => {
      newSquares[move.to] = {
        background:
          ...
          chessRef.current.get(move.to as any)?.color !==
            chessRef.current.get(square as any)?.color
  ...
  const onSquareClick = ({ square }: any) => {
    ...
    if (!moveFrom) {
      ...
      if (
        pieceOnSquare &&
        playerColor === turn &&
        pieceOnSquare.color === (playerColor === "white" ? "w" : "b")
      ) {
  ...
  useEffect(() => {
    ...
    const initSocket = () => {
      ...
      socket.on("game_update", (game: any) => {
        if (game.serverSentAt) {
          // ⏱️ ADD THIS BLOCK
          console.log(
            `[TIMING] Total propagation (move → opponent screen): ${Date.now() - game.serverSentAt}ms`,
          );
        }
        ...
        <div className="flex flex-col lg:flex-row gap-4 md:gap-6 max-w-7xl mx-auto">
          ...
          <div className="flex-1 flex flex-col gap-3 md:gap-4">
              ...
              <div className="flex items-center gap-2 md:gap-3">
                ...
                <div>
                  <div className="font-semibold text-xs md:text-base flex items-center gap-1.5 md:gap-2">
                    You{" "}
                    {playerColor && (
                      <span className="text-[8px] md:text-xs px-1.5 py-0.5 bg-blue-600/20 text-blue-400 rounded-full">
                        ({playerColor})
                      </span>
                    )}
                  </div>
            ...
            <div className="bg-gray-800/30 backdrop-blur-sm border border-gray-700 rounded-xl p-2 md:p-3 flex items-center justify-center gap-2 md:gap-3">
              <div
                className={`w-2.5 h-2.5 md:w-3 md:h-3 rounded-full ${chessRef.current.turn() === "w" ? "bg-white border-2 border-gray-400" : "bg-gray-900 border-2 border-gray-600"}`}
              />
          ...
          <div className="lg:w-80 flex flex-col gap-4">
            ...
            <div className="bg-gray-800/50 backdrop-blur-sm border border-gray-700 rounded-xl p-3 md:p-4">
              <h3 className="text-[10px] md:text-xs font-bold text-gray-400 mb-3 tracking-widest uppercase">
                Game Details
              </h3>
                ...
                {moveHistory.length === 0 ? (
                  ...
                  <div className="grid grid-cols-2 gap-x-4 gap-y-1">
                    {moveHistory.map((move, index) => (
                      ...
                      >
                        <span className="text-gray-500 w-4 font-mono">
                          {index % 2 === 0
